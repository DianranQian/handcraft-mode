	...
	factory: (require) => {
		...
		*/
		...
		const TOGGLES = [
			...
			{
				key: "memoryWriteTools",
				label: "记忆写入（默认开）",
				hint: "记住学习进度/偏好，跨会话持续教学"
			},
			{
				key: "memoryTools",
				label: "待办与目标（默认关）",
				hint: "dtodo / create_goal / update_goal"
			},
			{
				key: "codeSnippets",
				label: "代码演示（默认开）",
		...
		*/
		function HandcraftSection({ t, useHandcraft, load, set }) {
			...
			const value = state.value === null && draft === null ? null : {
				...state.value ?? {},
				...draft ?? {}
			};
			...
			const commit = (field, next) => {
				...
				timerRef.current = window.setTimeout(() => {
					...
					pendingRef.current = {};
					setDraft(null);
					for (const [key, v] of Object.entries(patch)) set(key, v);
				}, COMMIT_DEBOUNCE_MS);
			};
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", { children: [
				...
				/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("label", {
					...
					children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("input", {
						...
						disabled,
						onChange: (event) => {
							commit("enabled", event.target.checked);
						}
					...
					children: TOGGLES.map(({ key, label, hint }) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("label", {
						...
						children: [
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("input", {
								...
								disabled,
								onChange: (event) => {
									commit(key, event.target.checked);
								}
							}),
				...
				state.status !== "ready" && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
					style: { opacity: .5 },
					children: t("loading")
				})
			] });
